...
def cmd_hide(ctx: CLIContext, args: argparse.Namespace) -> int:
    ...
    for file_path, decision in files_to_process:
        ...
        if ctx.dry_run:
            ctx.log(f"  [DRY RUN] Would encrypt: {file_path} → {output_path}")
            processed_count += 1
            continue
...
def cmd_reveal(ctx: CLIContext, args: argparse.Namespace) -> int:
    ...
    for enc_path in target_paths:
        ...
        if ctx.dry_run:
            ctx.log(f"  [DRY RUN] Would reveal: {enc_path} → {output_path}")
            processed_count += 1
            continue
        ...
        try:
            if args.headers_only:
                ...
                ctx.log(f"  Metadata for {enc_path}:")
                ctx.log(f"    Chunk size: {metadata.chunk_size}")
                ctx.log(f"    Original size: {metadata.original_size}")
...
def cmd_explain(ctx: CLIContext, args: argparse.Namespace) -> int:
    ...
    ctx.log(colored(f"Rule evaluation for: {file_path}", Colors.BOLD))
    ctx.log("")
    ...
    ctx.log(f"  Profile:    {ctx.profile}")
    ctx.log(f"  Rule index: {decision.rule_index}")
    ...
    ctx.log("")
    ctx.log("  Actions:")
    ctx.log(f"    Scramble:    {decision.scramble}")
    ctx.log(f"    Encrypt:     {decision.encrypt}")
    ctx.log(f"    Chunk size:  {decision.chunk_size}")
